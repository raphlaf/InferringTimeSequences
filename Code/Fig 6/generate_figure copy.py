...
fig, axes = plt.subplots(3, 2, figsize=(3.4252, 5.2))
...
axes[0, 0].legend(bbox_to_anchor=[0.46, 1.0], loc="upper center")

axes[0, 1].set_ylabel(r"$P_\tau(\tau)$")
axes[0, 1].set_xlabel(r"$\tau$ $(s)$")
# axes[0, 1].text(0.80, 0.97, r"(b)", transform=axes[0, 1].transAxes, fontsize=12, fontweight='bold', va='top', ha='left',
#                 color='black', path_effects=[patheffects.withStroke(linewidth=1,foreground="white")])
panel_label(axes[0, 1], "(b)", loc="tr", fontsize=12, pad=3)
axes[0, 1].set_xticks([0.0, 30.0, 60.0])
...
axes[1, 0].plot(k_values, mean_tau[-1, :], "k--", label=r"$\sigma = 0$ $s$")
axes[1, 1].plot(k_values, xi_values[-1, :], "k--", label=r"$\sigma = 0$ $s$")
axes[1, 1].plot(k_values, xi_values[-2, :], "k:", label=r"Uniform")
# axes[1, 0].text(0.80, 0.97, r"(c)", transform=axes[1, 0].transAxes, fontsize=12, fontweight='bold', va='top', ha='left',
#                 color='black', path_effects=[patheffects.withStroke(linewidth=1,foreground="white")])
panel_label(axes[1, 0], "(c)", loc="tr", fontsize=12, pad=3)

axes[1, 0].set_ylabel(r"Optimal $\mu$ (s)")
axes[1, 1].set_ylabel(r"$\left<\right.$""CRLB"r"$\left.\right>_{\tau,T}$ (s)")
axes[1, 0].set_xlabel(r"$k$")
axes[1, 1].set_xlabel(r"$k$")
# axes[1, 1].text(0.80, 0.97, r"(d)", transform=axes[1, 1].transAxes, fontsize=12, fontweight='bold', va='top', ha='left',
#                 color='black', path_effects=[patheffects.withStroke(linewidth=1,foreground="white")])
...
for i in range(len(sigm_values)-1, -1, -1):
    ...
    p = np.exp(mu_values - sigma2)
    med = np.exp(mu_values)
    axes[2, 0].plot(k_values, mean_tau[i, :], label=sigm_labels[i])
    axes[2, 1].plot(k_values, xi_values[i, :], label=sigm_labels[i])


axes[2, 0].plot(k_values, mean_tau[-1, :], "k--", label=r"Single $\tau$")
axes[2, 1].plot(k_values, xi_values[-1, :], "k--", label=r"Single $\tau$")
axes[2, 1].plot(k_values, xi_values[-2, :], "k:", label=r"Uniform")

# axes[2, 0].text(0.80, 0.97, r"(e)", transform=axes[2, 0].transAxes, fontsize=12, fontweight='bold', va='top', ha='left',
#                 color='black', path_effects=[patheffects.withStroke(linewidth=1,foreground="white")])
panel_label(axes[2, 0], "(e)", loc="tr", fontsize=12, pad=3)

axes[2, 0].set_ylabel(r"Optimal $\mu$ (s)")
axes[2, 1].set_ylabel(r"rel-$\left<\right.$""CRLB"r"$\left.\right>_{\tau,T}$")
axes[2, 0].set_xlabel(r"$k$")
axes[2, 1].set_xlabel(r"$k$")
axes[2, 1].set_ylim(0.6444855763502046, 1.6)
# axes[2, 1].text(0.80, 0.205, r"(f)", transform=axes[2, 1].transAxes, fontsize=12, fontweight='bold', va='top', ha='left',
#                 color='black', path_effects=[patheffects.withStroke(linewidth=1,foreground="white")])
panel_label(axes[2, 1], "(f)", loc="br", fontsize=12, pad=3)
axes[2, 1].legend(bbox_to_anchor=[0.025, 1.0], loc="upper left")
